...
`colors.tsv` uses integer `color_id` plus `namespace` and `value` (`name` is accepted as an alias of `value`). A `color_set` cell is a comma-separated list of those ids, for example `0,1`. `labels.tsv` is `label_id`, `namespace`, `value`. Taxonomy, functional annotation, and sample colour stay in different namespaces. Sample colour is not a taxonomic label. `filter_colours` keeps a namespace or an explicit list of colour ids on a CFA, a CDBG, or a CGT. It does not change topology or features.
